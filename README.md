...
Dependency note: `jose` is temporarily pinned to 4.15.9 beneath `jwks-rsa@4.x`, whose CommonJS loader is incompatible with ESM-only `jose@6` in the current serverless runtime. Re-check Firebase Admin and `jwks-rsa` upstream releases periodically and remove the override once they provide a compatible resolution.
